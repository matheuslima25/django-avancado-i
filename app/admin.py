...
def marcar_como_verificado(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{queryset.count()} documentos marcados como verificados.")
...
class DocumentoAdmin(admin.ModelAdmin):
    list_display = ('arquivo_nome', 'usuario', 'categoria', 'data_envio', 'verificado', 'visualizar_arquivo')
    list_filter = ('categoria', TamanhoArquivoFilter, 'verificado')
    ...
    def visualizar_arquivo(self, obj):
        if obj.arquivo:
            return format_html(f"<a href='{obj.arquivo.url}' target='_blank'>Visualizar</a>")
        return "-"
